...
public class HelloFacebookSampleActivity extends FragmentActivity {
	...
	private Button postSelfieButton;
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == REQUEST_IMAGE_CAPTURE && resultCode == RESULT_OK) {
			...
			if(Util.bitmap!=null){
				Toast.makeText(this, "Picture taken successfully. Post in progress...", Toast.LENGTH_LONG).show();
				performPublish(PendingAction.POST_PHOTO_ON_PAGE, canPresentShareDialogWithPhotos);
			}
		}
	}
		...
		if (Session.getActiveSession()!=null && Session.getActiveSession().isOpened()) {
			...
			Request.newMeRequest(Session.getActiveSession(), new Request.GraphUserCallback() {
				...
				public void onCompleted(GraphUser user, Response response) {
					Log.e("Me", "Response : " + user);
					if (user != null && user.getId()!=null) {
						Log.e("Me", "UserID : " + user.getId());
						new Request(
								...
								new Request.Callback() {
									...
									public void onCompleted(Response response) {
										...
										if(response.getRawResponse()!=null){
											Log.e("Me", "UserID : " + response.getRawResponse());
											try {
	...
	private void onClickPublishPhotoOnPage() {
		...
		final ProgressBarDialog dialogBar = new ProgressBarDialog(this, "Cruising...");
		dialogBar.show();
		new Request(null, "/OSAF.FAA/photos", postParams, HttpMethod.POST, new Callback(){
			@Override
			public void onCompleted(Response response) {
				dialogBar.dismiss();
				...
				if(response.getRawResponse() != null && response.getRawResponse().contains("post_id"))
	...
	private boolean hasPublishPermission() {
		Session session = Session.getActiveSession();
		return session != null && session.getPermissions().contains("manage_pages");
	}
